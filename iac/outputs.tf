...
output "instance_public_ips" {
  ...
  value = {
    (var.frankfurt_region)  = oci_core_instance.cloud_vpn_instance.public_ip
...
output "ssh_connection_commands" {
  ...
  value = {
    (var.frankfurt_region)  = "ssh -i ${local_file.ssh_private_key.filename} ubuntu@${oci_core_instance.cloud_vpn_instance.public_ip}"
